  ...
  typeDeclaration(
    ...
    () => {
      const properties = propertiesFromFields(
        generator.context,
        Object.values(type.getFields()).map(v => {
          return {
            fieldName: v.name,
            responseName: v.name,
            ...v,
...
export function propertyDeclarations(
  ...
  properties.forEach(property => {
    if (
      isAbstractType(
        getNamedType((property.type || property.fieldType) as GraphQLType)
      )
    ) {
            ...
            ? property.fields.map(field => {
                if (field.fieldName === "__typename") {
                  const objectTypeName = getObjectTypeName(
                    (property.fieldType || property.type) as GraphQLType
                  );
                  return {
                    ...field,
                    typeName: objectTypeName,
...
function getPossibleTypeNames(
  ...
  return generator.context.schema
    .getPossibleTypes(getNamedType((property.fieldType ||
      property.type) as GraphQLType) as GraphQLAbstractType)
    .map(type => type.name);
}
